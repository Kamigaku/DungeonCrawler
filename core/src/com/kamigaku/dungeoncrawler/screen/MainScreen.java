...
public class MainScreen implements Screen {
    ...
    private final BitmapFont activeFont;
    ...
    public void render(float delta) {
        ...
        for(int i = 0; i < options.length; i++) {
            if(currentChoice == i)
                activeFont.draw(ScreenManager.game.batch, options[i],
                            (Gdx.graphics.getWidth() / 2) - 50, 
                            (Gdx.graphics.getHeight() / 2) - (50 * i));
            else
                defaultFont.draw(ScreenManager.game.batch, options[i], 
                            (Gdx.graphics.getWidth() / 2) - 50, 
                            (Gdx.graphics.getHeight() / 2) - (50 * i));
        }
